...
// Get cleaner workload for smart assignment
router.get('/cleaners/workload', async (req, res) => {
  try {
    const cleaners = await Cleaner.find();
    const workload = await Promise.all(cleaners.map(async (cleaner) => {
      const activeTasksCount = await Complaint.countDocuments({
        assignedCleanerId: cleaner._id.toString(),
        status: 'Assigned'
      });
      return {
        _id: cleaner._id,
    ...
    // Sort by lowest activeTasksCount
    workload.sort((a, b) => a.activeTasksCount - b.activeTasksCount);
    res.json(workload);
  } catch (error) {
    console.error('Error fetching cleaner workload:', error);
...
router.get('/buildings', async (req, res) => {
  try {
    const buildings = await Building.find({}, '_id name').sort({ name: 1 });
    res.json(buildings);
  } catch (error) {
...
router.post('/waste', async (req, res) => {
  try {
    const { buildingId, wetWeight, dryWeight, rejectWeight, cleanerId } = req.body;
    ...
    const reject = parseFloat(rejectWeight) || 0;

    if (wet <= 0 && dry <= 0 && reject <= 0) {
      return res.status(400).json({ error: 'At least one waste weight must be greater than 0' });
    }

    const loggedBy = cleanerId || 'System';

    if (wet > 0) {
      await WasteLog.create({ buildingId, loggedBy, wasteType: 'Wet', weightKg: wet });
    }
    if (dry > 0) {
      await WasteLog.create({ buildingId, loggedBy, wasteType: 'Dry', weightKg: dry });
    }
    if (reject > 0) {
      await WasteLog.create({ buildingId, loggedBy, wasteType: 'Reject', weightKg: reject });
    }

    await updateGreenScore(buildingId);

    res.status(201).json({ message: 'Daily waste logged successfully' });
  } catch (error) {
    console.error('Error creating daily waste logs:', error);
...
router.post('/complaints', async (req, res) => {
  try {
    const { buildingId, description, raisedBy, category } = req.body;

    if (!buildingId || !description || !raisedBy || !category) {
      return res.status(400).json({ error: 'buildingId, description, raisedBy, and category are required' });
    }
...
router.get('/complaints/cleaner/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const complaints = await Complaint.find({ assignedCleanerId: id })
      .sort({ createdAt: -1 })
      .populate('buildingId', 'name address');
    ...
    console.error('Error fetching cleaner complaints:', error);
    res.status(500).json({ error: 'Failed to fetch cleaner complaints' });
  }
});
...
router.post('/warnings', async (req, res) => {
  try {
    const { buildingId, buildingName, alertType, message, additionalNote } = req.body;
...
router.get('/warnings/building/:buildingId', async (req, res) => {
  try {
    const warnings = await Warning.find({ buildingId: req.params.buildingId }).sort({ createdAt: -1 });
    res.json(warnings);
  } catch (error) {
...
router.put('/warnings/:id/read', async (req, res) => {
  try {
    const warning = await Warning.findByIdAndUpdate(req.params.id, { status: 'Read' }, { new: true });
...
router.post('/auth/verify', async (req, res) => {
  try {
    ...
    if (email === 'admin@bmc.gov') {
      return res.status(200).json({ role: 'BMC_ADMIN' });
    }

    const building = await Building.findOne({ email });
    if (building) {
      return res.status(200).json({ role: 'BUILDING_ADMIN', id: building._id });
    }

    const cleaner = await Cleaner.findOne({ email });
    if (cleaner) {
      return res.status(200).json({ role: 'CLEANER', id: cleaner._id });
    }
